...
export default class SendApp extends React.Component {
  ...
  getDefaultAccount = (accountIndex) => {
    ...
    return makeAccountContactOption(account.name, [smtpUsername, smtpUsername]);
  };

  getEmailData(isDraft = false) {
    const emailData = _.pick(this.state, ["subject", "attachments"]);
    ...
    if (this.state.selectedSignatureIdx > -1) {
      const signatureHtml =
        this.props.signatures[this.state.selectedSignatureIdx].html;
      const signatureText =
        this.props.signatures[this.state.selectedSignatureIdx].text;
  ...
  renderQuote() {
    ...
    return (
      <div>
        <div id="include-quote" className={this.state.sending ? "sending" : ""}>
          <input
            id="include_quote"
            type="checkbox"
  ...
  renderAddressBookSelect(contactOptions, dataKey) {
    function delayedOptionLoad(inputValue) {
      ...
      if (inputValue.length > 2) {
        ...
        filteredOptions = _.filter(contactOptions, (option) =>
          option.lowercaseLabel.includes(inputValue)
        );
  ...
  renderSendButton() {
    ...
    if (this.state.isSending) {
      if (this.state.saveError) {
        ...
        text = (
          <span>
            <i className="fa fa-spin fa-refresh" /> Sending email
          </span>
        );
        classes.push("disabled");
      }
  ...
  renderAttachButton() {
    ...
    return (
      <Tooltip text={attachments} extraTop={16}>
        {button}
      </Tooltip>
    );
  }
  ...
  render() {
    const accountOptions = _.reduce(
      ...
      (memo, account, accountIndex) => {
        ...
        if (accountContacts && accountContacts.length > 0) {
          return _.concat(
            memo,
            _.map(accountContacts, (nameEmail) =>
              makeAccountContactOption(account.name, nameEmail)
            )
          );
        }
    ...
    const signatureOptions = _.map(this.props.signatures, (signature, i) => ({
      value: i,
      ...
      >
        ...
        >
          <div className="flex form-top" onClick={stopEventPropagation}>
            ...
            <div className="wide">
              ...
              <Select
                ...
                onChange={_.partial(this.handleSelectChange, "accountContact")}
              />
            </div>
          ...
          <div className="flex form-content" onClick={stopEventPropagation}>
            <div className="wide">
              <div id="textarea-body" className={editorClasses.join(" ")}>
                <Editor
            ...
            <div className="wide signature">{this.renderSignature()}</div>

            <div className="wide quote">{this.renderQuote()}</div>
          </div>
        </form>
      </section>
    );
