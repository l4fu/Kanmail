        ...
        this.setState({
          newAccountError: (
            <span>
              ...
              . Provider error:
              <br />
              <code>{data.json.error_message}</code>
            </span>
          ),
...
class OauthAccountFormMixin extends GenericAccountForm {
  ...
  checkForOauthRequest = () => {
    ...
    get(`/api/oauth/response/${this.state.oauthRequestId}`).then(
      (oauthData) => {
  ...
  renderTitle() {
    return (
      <h3>
        <img src={gmailLogo} /> Add Gmail Account
      </h3>
    );
  }
...
class GmailAccountForm extends OauthAccountFormMixin {
  ...
  renderTitle() {
    return (
      <h3>
        <img src={gmailLogo} /> Sign in with Google
      </h3>
    );
  }
}
