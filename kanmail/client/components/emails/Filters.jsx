...
function getSidebarFolderLink(folderName) {
  return subscribe(getSidebarFolderLinkStore(folderName))(SidebarFolderLink);
}
...
export default class Filters extends React.Component {
  ...
  renderShowAllFolders() {
    ...
    const sidebarFoldersLength =
      this.props.styleSettings.sidebar_folders.length || 0;
  ...
  renderOtherFolderLinks() {
    ...
    const sidebarFolders = this.renderFolderLinks(sidebarFolderNames, {
  ...
  renderUpdateLink() {
    if (this.props.updateError) {
      return (
        <li>
          <a onClick={() => openLink("https://kanmail.io/download")}>
            <span className="update error">
              Error downloading update: <code>{this.props.updateError}</code>.
              <br />
