...
export default class ThreadMessage extends React.Component {
  ...
  processMessageDOM() {
    ...
    _.each(doc.querySelectorAll("blockquote"), (blockquote) => {
      ...
      blockquote.parentNode.insertBefore(showButton, blockquote.nextSibling);
    });
  ...
  handleClickAlwaysShowImages = () => {
    const requests = this.getFromEmailAddresses().map((email) =>
      put(`/api/contacts/allow-images/${email}`)
    );
  ...
  handleClickHideImages = () => {
    const requests = this.getFromEmailAddresses().map((email) =>
      delete_(`/api/contacts/allow-images/${email}`)
    );
  ...
  renderBody() {
    const { body, account_name, folder_name, uid } = this.props.message;
    ...
    _.each(doc.querySelectorAll("img"), (img) => {
      ...
      img.src = `/api/emails/${account_name}/${encodeFolderName(
        folder_name
      )}/${uid}/${contentIds[cid]}?Kanmail-Session-Token=${
        window.KANMAIL_SESSION_TOKEN
      }`;
    });
